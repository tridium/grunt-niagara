...
function allBuiltJs() {
  return allFilesWithSrc('**/*.built.min.js');
}
...
function getSourceMappings(grunt) {
  const {
    ...
    testBuiltJs = allBuiltJs().from('build/src/rc').to('build/src/rc')
  } = grunt.config.getRaw('sourceMappings') || {};
  return { source, resources, ext, test, testResources, testExt, testBuiltJs };
}
