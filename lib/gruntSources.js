...
function getSourceMappings(grunt) {
  const {
    ...
    extTest = allFiles().from('srcTest/ext').to('build/srcTest/ext')
  } = grunt.config.getRaw('sourceMappings') || {};
  return { source, resources, ext, test, testResources, extTest };
}
 ...
 * @property {GruntFileMapping} extTest - external code and dependencies used in
